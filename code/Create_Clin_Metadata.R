...
treat_path_foo <- treat_path %>%
  ...
  # Convert all columns to numeric (will coerce 'Indeterminate' and '.' to NA)
...
taq_data <- taq_data %>%
  select(STUDYID, contains("either"))
treat_full <- treat_full %>%
  left_join(., taq_data, by = c("STUDY_ID" = "STUDYID"))
