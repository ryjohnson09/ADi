#####################################################
# Name: EAEC_linear_plot_PanEC_IgA_v15_LLSseverity.R
# Author: Ryan Johnson
...
PanEC_IgA_spread <- PanEC_IgA %>%
  ...
  select(ID, Description, Patients, visit, norm_value, LLS_severity) %>%
...
one_v_five$LLS_severity <- factor(one_v_five$LLS_severity, levels = c("mild", "moderate", "severe"))
...
PanEC_IgA_plot <- ggplot(data = one_v_five, aes(x = `1`, y = `5`)) +
  ...
  labs(title = "PanEC IgA - EAEC Infected Patients: Visit 1 vs Visit 5",
       ...
       caption = "LLS severity in past 8 hrs: 0-1 mild, 2-4 moderate, 5+ severe") +
  theme_minimal() +
  theme(
...
## Save Plots -----------------------------------------------------------
ggsave(plot = PanEC_IgA_plot, "results/figures/EAEC_linear_PanEC_IgA_v15_LLSseverity.png", width = 14, height = 8)
