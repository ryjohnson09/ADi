...
#  acute to convalescent using the PanEC_IgG or IgA
#  data sets
#####################################################
...
PanEC_IgG_plot <- ggplot(data = one_v_five, aes(x = `1`, y = `5`)) +
  ...
  geom_point(data = pick(~ID_Description %in% top_10_mean_diff), 
             aes(color = factor(ID_Description, levels = top_num_mean_diff))) +
  facet_wrap(~Patients) +
  labs(title = "EAEC Infected Patients: Visit 1 vs Visit 5",
...
## Save Plots -----------------------------------------------------------
ggsave(plot = PanEC_IgG_plot, "results/figures/EAEC_linear_plot_v15.png", width = 14, height = 8)
