...
taq_data_adi_either <- taq_data_adi %>% 
  # CMY
  ...
  # Arrange
  select(STUDYID, visit_number, CMY_Card, CMY_Stool, CMY_either,
...
taq_data_adi_either <- taq_data_adi_either %>% 
  # Change visit to V1 and V5
  mutate(visit_number = ifelse(visit_number == 1, "V1",
                        ifelse(visit_number == 5, "V5", NA))) %>% 
  gather(key = Target, value = taq_value, -STUDYID, -visit_number) %>% 
  unite(Target, c("Target", "visit_number")) %>%
  spread(key = Target, value = taq_value) %>% 
  # Remove card_V5
  select(-ends_with("card_V5"))


# Write to data/processed -----------------------------------
write_csv(x = taq_data_adi_either, path = "data/processed/Taq_tidy.csv")
